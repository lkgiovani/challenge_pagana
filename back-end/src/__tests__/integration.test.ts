...
describe('POST /api/messages', () => {
  ...
  it('retorna 400 quando conversa está em status transferred', async () => {
    vi.mocked(runAgent).mockResolvedValue({
      transfer: true,
      reply: 'Transferindo',
      intent: 'suporte',
    ...
    await app.inject({ method: 'POST', url: '/api/messages', payload: { conversationId, message: 'bug' } })
    const response = await app.inject({
      method: 'POST',
      url: '/api/messages',
  ...
  it('atendente envia mensagem quando conversa está em status in_progress', async () => {
    ...
    const response = await app.inject({
      method: 'POST',
      url: '/api/messages',
      payload: { conversationId, message: 'Olá, como posso ajudar?' },
    })
    expect(response.statusCode).toBe(200)
    expect(response.json().reply).toBe('Olá, como posso ajudar?')
    expect(response.json().transfer).toBe(false)
    expect(response.json().intent).toBeNull()
  })
